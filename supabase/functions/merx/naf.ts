// Le libellé officiel d'une activité, pour l'écrire en français.
//
// L'annuaire de l'État ne rend que le code : « 21.20Z ». Le commercial, lui, lit
// « Fabrication de préparations pharmaceutiques ». Afficher le code dans la colonne
// Activité, c'était lui donner à déchiffrer ce qu'on avait justement à traduire.
//
// La table est la nomenclature d'activités française rév. 2 en entier : sept cent
// trente-deux sous-classes, celles-là mêmes que publie l'annuaire, plus les
// quatre-vingt-huit divisions pour les codes tronqués. Aucun code ne manque, donc
// aucune fiche ne retombe sur son numéro.

/** Sous-classes « 21.20Z » : le format que rend l'annuaire. */
...
/** Divisions « 21 » : le repli quand le code arrive tronqué. */
...
/** L'activité en clair, ou rien si le code est vide ou inconnu — on ne devine pas. */
export function libelleNaf(naf: string | null): string | null {
  const code = (naf ?? "").trim().toUpperCase();
  if (!code) return null;
  return SOUS_CLASSES[code] ?? DIVISIONS[code.slice(0, 2)] ?? null;
}
