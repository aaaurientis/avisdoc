...
// Libellés lisibles pour les champs à substituer.
const LIBELLES: Record<string, string> = {
  "{{date}}": "Date",
  "{{lieu}}": "Lieu",
  "{{lien}}": "Lien d'inscription",
...
export function Emails({ entreprise }: { entreprise: string }) {
  ...
  const option =
    sequence?.options.find((o) => o.id === optId) ?? sequence?.options[0];

  function substituer(texte: string): string {
    let out = texte;
  ...
  const corps = useMemo(
    () => (option ? option.corps.map(substituer) : []),
    [option, valeurs]
  );
  ...
  return (
    <div>
      <h1 className="font-serif text-2xl">E-mails de campagne</h1>
      <p className="mt-1 text-sm text-ardoise">
        Renseignez les champs, choisissez la séquence et l'angle, puis copiez le
        message dans votre messagerie.
      </p>

      <div className="mt-6 grid gap-6 md:grid-cols-[280px_1fr]">
        {/* Champs + sélection */}
        <div className="space-y-4">
          <div className="space-y-3 rounded-lg border border-filet bg-white p-4">
            {tokens.map((t) => (
              <label key={t} className="block text-sm">
                <span className="text-ardoise">{LIBELLES[t] ?? t}</span>
                <input
                  value={valeurs[t]}
                  onChange={(e) => setValeurs({ ...valeurs, [t]: e.target.value })}
                  className="mt-1 w-full rounded border border-filet bg-white px-2 py-1.5"
                />
              </label>
            ))}
          </div>
          ...
          <div className="space-y-3 rounded-lg border border-filet bg-white p-4">
            <label className="block text-sm">
              <span className="text-ardoise">Séquence</span>
              <select
                value={seqId}
                onChange={(e) => {
                  setSeqId(e.target.value);
                  const s = DATA.sequences.find((x) => x.id === e.target.value);
                  setOptId(s?.options[0]?.id ?? "");
                }}
                className="mt-1 w-full rounded border border-filet bg-white px-2 py-1.5"
              >
            ...
            <label className="block text-sm">
              <span className="text-ardoise">Angle</span>
              <select
                value={optId}
              ...
              >
                {sequence?.options.map((o) => (
                  <option key={o.id} value={o.id}>{o.angle}</option>
                ))}
              </select>
            </label>
          </div>
        ...
        {/* Aperçu */}
        <div className="rounded-lg border border-filet bg-white p-5">
          <div className="flex items-start justify-between gap-4">
            <p className="font-medium">
              <span className="text-ardoise">Objet : </span>{objet}
            </p>
            <button
              onClick={copier}
              className="shrink-0 rounded bg-cyan px-3 py-1.5 text-sm font-medium text-marine"
            >
              {copie ? "Copié ✓" : "Copier"}
            </button>
          </div>
