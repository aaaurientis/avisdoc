...
BUCKET = {"public": "espace-docs-public", "client": "espace-docs-client"}
BUCKET_LOGOS = "espace-logos"
SANS_LOGO = "sans-logo"
...
def generer_pour_client(job_id: str, client_id: str) -> dict:
    ...
    logo_bytes = sb.download_object(BUCKET_LOGOS, client["logo_path"])
    valider_logo(logo_bytes)
    empreinte = sha256(logo_bytes)

    # Le nom d'entreprise (company) sert de suffixe aux fichiers : on le
    # « slugifie » pour éviter espaces/accents dans les noms de fichiers.
    nom = re.sub(r"[^a-z0-9]+", "-", str(client["company"]).lower()).strip("-") or "client"
    suffixe = "-" + nom
