...
def capture(monkeypatch):
    ...
    monkeypatch.setattr(sb, "get_client", lambda cid: {
        "id": cid, "company": "ACME", "logo_path": f"{cid}/logo.png",
...
def test_repartition_et_cloisonnement(capture):
    ...
    for row in capture["inserts"]:
        attendu = "espace-docs-public" if par_id[row["doc_catalogue_id"]]["acces"] == "public" \
            else "espace-docs-client"
...
def test_logo_invalide_rejete(monkeypatch):
    ...
    monkeypatch.setattr(sb, "get_client", lambda cid: {
        "id": cid, "company": "ACME", "logo_path": f"{cid}/logo.png", "logo_sha256": "x",
    })
